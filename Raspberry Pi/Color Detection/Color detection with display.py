...
def color_detection(side, clock):
    ...
    if create_contour(yellow_mask) and clockCheck(clock):
        if side == 'R':
            ser.write(b"rYS\n")
            print("Yellow color detected on right")
        else:
            ser.write(b"lYS\n")
            print("Yellow color detected on left")
        clock = time.time()
    elif create_contour(green_mask) and clockCheck(clock):
        if side == 'R':
            ser.write(b"rGU\n")
            print("Green color detected on right")
        else:
            ser.write(b"lGU\n")
            print("Green color detected on left")
        clock = time.time()
    elif create_contour(red_mask) and clockCheck(clock):
        if side == 'R':
            ser.write(b"rRH\n")
            print("Red color detected on right")
        else:
            ser.write(b"lRH\n")
            print("Red color detected on left")
        clock = time.time()
    return clock
